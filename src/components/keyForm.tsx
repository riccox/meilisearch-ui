...
import { Input, Select, DatePicker } from '@douyinfe/semi-ui';
import _ from 'lodash';
import { KeyCreation } from 'meilisearch';
...
type KeyFormValue = KeyCreation & Partial<Pick<KeyCreation, 'actions' | 'indexes' | 'expiresAt'>>;

export const KeyForm: FC<Props> = ({ className = '', type = 'create', data, afterSubmit }) => {
        ...
        case 'create':
          await client.createKey({
            ...
            expiresAt: _.isEmpty(values.expiresAt)
              ? null
              : getTimeText(values.expiresAt, { format: 'YYYY-MM-DD HH:mm:ss+00:00' }),
          });
          break;
        case 'edit':
  ...
  return (
    <form
      className={cn(className, `flex flex-col gap-y-6 w-full p-4 bg-white flex-1 rounded-t-6`)}
      onSubmit={form.handleSubmit(onSubmit)}
    >
      <p className={`text-center font-semibold text-lg`}>{t(`form.title.${formType}`)}</p>
          ...
          rules={{
            validate: (value: Date | null) => {
              if (value && dayjs(value).isValid()) {
                ...
                form.setError('expiresAt', { message: t('form.expireAt.invalid') });
                return false;
              }
            },
          }}
