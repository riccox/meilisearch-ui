		...
		},
		{
			...
			render: (_, item) => {
				return (
					<TagGroup
						maxTagCount={3}
						tagList={item.indexes.map((i) => ({ children: i }))}
						showPopover
					/>
				);
			},
		},
		{
			...
			render: (_, item) => {
				return (
					<TagGroup
						maxTagCount={3}
						tagList={item.actions.map((i) => ({ children: i }))}
						showPopover
					/>
				);
			},
							...
							Modal.info({
								...
								content: (
									<Descriptions className="pb-3">
										<Descriptions.Item itemKey="UID">
											{record.uid}
										</Descriptions.Item>
										<Descriptions.Item itemKey={t("description")}>
											{record.description}
										</Descriptions.Item>
	...
	return (
		<div className="flex-1 overflow-scroll max-h-fit">
			<main className="p-4 flex flex-col gap-4">
				...
				<Table
					columns={columns}
					dataSource={keysQuery.data?.results}
					pagination={false}
					empty={t("empty")}
				/>
				...
				>
					<KeyForm
						...
						data={
							formState.editing
								? { ...formState.editing, name: formState.editing.name ?? "" }
								: undefined
						}
						type={formState.type}
					/>
